...
import CustomLineChart from '../StatsUtils/ReverseLineChart'
...
export default class Rank extends React.Component {
	...
	render() {
		...
		for(i = 0; i < rank.length; i++){
			if(rank[i] > highestValue){
				highestValue = rank[i];
			}
		}

		for(i = 0; i < rank.length; i++){
			rank[i] = highestValue-rank[i];
		}
		highestValue = 0;
		for(i = 0; i < rank.length; i++){
			if(rank[i] > highestValue){
		...
		return (
            <View style={styles.container}>
				...
				<CustomLineChart highest={highestValue} data={rank} width={scale(250, parentWidth, Dimensions.get('window').width)} height={scale(250, parentHeight, Dimensions.get('window').height)}/>
